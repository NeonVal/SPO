...
vector<string> C, L;
vector<ObI> I;
vector<string> K = { "and", "array", "boolean", "break", "case", "char", "declare", "default", "do", "else", "end", "false", "for", "if", "integer", "let", "main", "not", "of", "or", "program", "read", "repeat", "then", "true", "until", "void", "while", "write", "xor" };
vector<string> R = { "!", "#", "$", "%", "&", "'", "(", ")", "*", "+", ",", "-", ".", "/", ":", ";", "<", "=", ">", "?", "[", "]", "^", "{", "|", "}" };
vector<string> D = { "!=", "&&", "*=", "++", "+=", "--", "-=", "/*", "//", "/=", "<<", "<=", "<>", "==", ">=", ">>", "||" };
...
//{
//	Token temp = scan(true);
//	if ((temp.ch == 'R') && (temp.n == 8 || temp.n == 12))
//	{
//		temp = scan(false);
//		if (!F())
//			return false;
//		temp = scan(true);
//		if ((temp.ch == 'R') || (temp.ch == 'D') || (temp.ch == 'I') || (temp.ch == 'C'))
//			if (!T2())
//				return false;
//	}
//	return true;
...
//{
//	Token temp = scan(true);
//	if ((temp.ch == 'R') && (temp.n == 9 || temp.n == 10))
//	{
//		temp = scan(false);
//		if (!T())
//			return false;
...
{
	Token temp = scan(false);
	if (temp.ch == 'K' &&  temp.n == Find(K, "declare"))
	{
		asm1 << "; user data\n";
		temp = scan(true);
		do
...
bool Repeat()
{
	...
	if (temp.ch == 'R' && temp.n == Find(R, "("))
	{
		...
		mainCode.push_back("pop ax\n");
		mainCode.push_back("cmp ax, 0\n");
		mainCode.push_back("jz " + repeatMark + "\n");
		mainCode.push_back("jmp " + endMark + "\n");
		mainCode.push_back(endMark + ":\n");
		return true;
	}
